#Script to extract meds, labs, and CPT codes from MIMIC2 for predictive analytics
#
#Author: 	Jeremy L. Warner M.D., M.S.
...
#	use without limitations. The original author is not responsible for any errors
#	associated with the script and offers no guarantees as to its functionality.
...
cat('Temporal Phenome Extraction, Version 14.1',
    file='log.txt',
    sep='\n',
    append=TRUE)
...
#Clean up
rm(all.demo, all.eth, all.hadm.icu, all.temp, elix, last.lab)
...
if(include.expanded)
{
  ...
  temp1 <- comp.candidates.expanded[comp.candidates.expanded$codec != '',1:3]
  temp2 <- comp.candidates.expanded[,5:6]
  colnames(temp2) <- c('codec','description')
  temp2$HAC <- 1
...
if(include.maybes)
	foo.icd9 <- comp.candidates$codec[comp.candidates$HAC %in% c(1:2)] else
	foo.icd9 <- comp.candidates$codec[comp.candidates$HAC == 1]
...
cat(paste('Possible complications were included (T/F):', include.maybes),
    ...
    sep='\n',
    append=TRUE)
cat('List of included complications:',
    file='log.txt',
    sep='\n',
    append=TRUE)
cat(foo.icd9,
...
foo.icd9 <- paste("'", foo.icd9, "',", sep="", collapse=" ")
foo.icd9 <- substr(foo.icd9, start = 1, stop = nchar(foo.icd9) - 1)

cases.all <- dbGetQuery(conn, paste("
	SELECT distinct hadm_id 
	FROM mimic2v26.icd9 
	WHERE code in (", foo.icd9, ")"))

cases.all <- all.hadm[which(all.hadm$hadm_id %in% cases.all$hadm_id),]
...
controls <- controls[sampler[c(1:round(c.match*dim(cases)[1]))],]
...
#3.5 Add a case or control designator for Weka, later
cases <- cbind(Case='Y',cases)
controls <- cbind(Case='N',controls)

#3.6 Combine the cases and controls into one dataframe
...
for (interval in c(1,2,3,6,12,18,24,48,72,96,120)) #Need to add loop 1
{  
...
names(cohort) <- c('Case','hadm_id','gender','age','ethnicity','elix.index')
...
#				} else patient[1,'any.ppi.h2.rx'] <- F
    ...
    #4.3.8 Add the patient to the cohort table 
		...
		cohort[i,c(1:6,which(colnames(cohort) %in% colnames(patient)))] <- 
			cbind(combo[i,c('Case','hadm_id','gender','age','ethnicity','elix.index')],patient)
...
cat('',
	...
	sep='\n',
	append=TRUE)
cat(paste('Total included cases:',sum(combo$Case=='Y')),
	file='log.txt',
	sep='\n',
	append=TRUE)
cat(paste('Total included controls:',sum(combo$Case=='N')),
	file='log.txt',
	sep='\n',
	append=TRUE)
...
save(all.hadm,cohort,
	file = paste("temporal.extract.v14.1.extended.first.",interval,".hours.RData",sep=""))
