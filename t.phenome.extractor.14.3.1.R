...
cohort <- data.frame(t(rep(NA,7)))
names(cohort) <- c('category','hadm_id','gender','age','ethnicity','elix.index', 'icd9string')
...
#				} else patient[1,'any.ppi.h2.rx'] <- F
    ...
    #4.3.8 Add the patient to the cohort table 
		...
		cohort[i,c(1:7,which(colnames(cohort) %in% colnames(patient)))] <- 
			cbind(combo[i,c('category','hadm_id','gender','age','ethnicity','elix.index','icd9string')],patient)
...
# 5. In section 4.2, changed 6 to 7, and added 'icd9string'
# 6. In section 4.3.8, changed 6 to 7, and added 'icd9string'
#
